...
states: Tuple[Tuple[status, vm_state, Optional[task_state], int], ...] = (
    ...
    # idle off
    ("SHUTOFF", "stopped", None, ERR),
    # powering-on
    ("SHUTOFF", "stopped", "powering-on", ERR),
    # powering-off
    ...
    # reboot-hard
    ("ACTIVE", "active", "reboot_pending_hard", OK),
    ("ACTIVE", "active", "reboot_started_hard", OK),
    ("ACTIVE", "active", "rebooting_hard", OK),
)


@pytest.mark.parametrize(("status", "vm_state", "task_state", "result"), states)
def test_power_cycle_reboot_hard(
