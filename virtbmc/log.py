...
def log_format(
    ...
    message_format: str = "{asctime} {levelname}: [{name}] {message}",
    ...
    if sys.stderr.isatty() and not os.environ.get("NO_COLOR", False):
        ...
        class ColorFormatter(logging.Formatter):
            def format(self, record: logging.LogRecord) -> str:
                ...
                _formatter = logging.Formatter(fmt=_format, style="{", datefmt=date_format)
                return _formatter.format(record)

        formatter = ColorFormatter()
    else:
        formatter = logging.Formatter(fmt=message_format, style="{", datefmt=date_format)
